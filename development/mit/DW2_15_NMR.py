...
def do_integrate(signal):
    I_benzene = ca.analysis.integrate.integrate_trapz(signal, x_range=(7.1, 7.45))
    I_bond = ca.analysis.integrate.integrate_trapz(signal, x_range=(5.6, 6.6))
    I_mono = ca.analysis.integrate.integrate_trapz(signal, x_range=(3.4, 3.75))
    return I_benzene, I_bond, I_mono


def integrate_array(nmr_array: ca.nmr.NMRSignal2D):
    for signal in nmr_array.signal_iter():
        I_benzene, I_bond, I_mono = do_integrate(signal)
    ...
    fig = go.Figure(go.Scatter(x=nmr_array.y, y=conv,
                             text=[f'X: {x}, Index: {i}' for i, x in enumerate(nmr_array.y)],
                             hoverinfo='text',
                             ))
    fig.write_html("conv.html", auto_open=True)
...
def main():
    ...
    nmr_array = ca.nmr.NMRSignal2D.from_feather(path)

    # process data
    # nmr_array.processor.add(ca.processing.translations.AlignMax(range_=(7.2, 7.4), x_value=7.3, temporal_processing=True))
    nmr_array.processor.add(ca.processing.smoothing.Gaussian(sigma=5, temporal_processing=True))
