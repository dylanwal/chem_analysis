...
from chem_analysis.analysis.peaks.peak_integration import base_integrate
...
class PeakPropertiesData(PeakProperties):
    ...
    def _get_y_norm(self) -> np.ndarray:
        if self._y_norm is None:
            self._y_norm = self.parent.y/np.trapezoid(x=self.parent.x, y=self.parent.y)
    ...
    @property
    def area(self) -> float:
        return base_integrate(self.parent.x, self.parent.y, self.parent.to_zero, checks=True)

    # the following properties are more expensive to compute, so require call
    # def mean(self) -> float:
